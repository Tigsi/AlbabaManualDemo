...
2） 大于 0 但小于 size，重新创建大小等于 size 的数组，增加 GC 负担。 解释：如果传入的数组大小小于集合元素的数目，复制前会创建大小合适的新数组，原来的空数组就没用了，空数组的回收会增加 gc 的负担。
3） 等于 size，在高并发情况下，数组创建完成之后，size 正在变大的情况下，负面影响与 2 相同。解释：size 正在变大，说明 len 会小于 size 情况同 2。
